...
//Should I have to modify those sec to even more?

// ADC/DAC sampling rate in Hz
//#define AUDIO_SAMPLING_RATE		1000
#define AUDIO_SAMPLING_RATE	  12000
//Should I have to modify these in 12000?
			...
			switch(Demo.chBtn) {
				...
				case 'd':
					if (!Demo.fAudioRecord && !Demo.fAudioPlayback)
					{
						...
						fnSetHpOutput();
						///////PRINTRECORDED//////////
						for(int u = 0; u < NR_AUDIO_SAMPLES*5; u+=5){
													xil_printf("\n %d", u);
													xil_printf("\n %x", ((uint8_t)(MEM_BASE_ADDR+(u))));
													xil_printf("\n %x", ((uint8_t)(MEM_BASE_ADDR+(u+1))));
													xil_printf("\n %x", ((uint8_t)(MEM_BASE_ADDR+(u+2))));
													xil_printf("\n %x", ((uint8_t)(MEM_BASE_ADDR+(u+3))));
													xil_printf("\n %x", ((uint8_t)(MEM_BASE_ADDR+(u+4))));
												}
						/////////////////////////////////////////////////
						//TEST/////////////////////////////////////////////////////////
						//
						/*int p=0;
						for(int u = 0; u < sizeof(ABEEP)/4; u++){
							if (ABEEP[u] < 0){
								((uint8_t)(MEM_BASE_ADDR+(p)))= 0xFF;
							}else{
								((uint8_t)(MEM_BASE_ADDR+(p)))= 0x00;
							}
							((uint32_t)(MEM_BASE_ADDR+(p+1)))=ABEEP[u];
							p = p + 5;
						}*/
						//fnAudioPlay2(sAxiDma,sizeof(ABEEP)/4, MEM_BASE_ADDR);
						//////////////////////////////////////////////////////////////
						//fnAudioPlay2(sAxiDma,sizeof(ABEEP)/4, &ABEEP);
				...
				case 'l':
					if (!Demo.fAudioRecord && !Demo.fAudioPlayback)
					{
						//  if you remove Demo.fAudioRecord. it can start play without running record Clear thanks!!

						xil_printf("\r\nStart Playback...");
						fnSetLineOutput();
						///////PRINTRECORDED//////////
						for(int u = 0; u < 20; u+=5){
											xil_printf("\n %d", u);
										    xil_printf("\n %x", ((uint8_t)(MEM_BASE_ADDR+(u))));
											xil_printf(" %x", ((uint8_t)(MEM_BASE_ADDR+(u+1))));
										    xil_printf(" %x", ((uint8_t)(MEM_BASE_ADDR+(u+2))));
										    xil_printf(" %x", ((uint8_t)(MEM_BASE_ADDR+(u+3))));
											xil_printf(" %x\n", ((uint8_t)(MEM_BASE_ADDR+(u+4))));
												}
						/////////////////////////////////////////////////
						//TEST/////////////////////////////////////////////////////////
						//
						...
						//fnAudioPlay2(sAxiDma,sizeof(ABEEP)/2, MEM_BASE_ADDR);
						//////////////////////////////////////////////////////////////

						//fnAudioPlay2(sAxiDma,sizeof(ABEEP)/4, &ABEEP);
						fnAudioPlay(sAxiDma,NR_AUDIO_SAMPLES);

						//fnAudioPlay(sAxiDma,sizeof(ABEEP)); // the length should be match with the buffer
						Demo.fAudioPlayback = 1;
					}
					else
					{
